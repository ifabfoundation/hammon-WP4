...
from PIL import Image, ImageDraw
from util.ls_filter import ls_filter
from util import normalize, unnormalize
from vanishing_points_utils.zl_predict import zl_predict
from vanishing_points_utils.z_predict import z_predict
from vanishing_points_utils.hl_predict import hl_predict
from vanishing_points_utils.hl_score import hl_score
from vanishing_points_utils.hl_sample import hl_sample
from vanishing_points_utils.vp_predict import vp_predict
...
def Refine(im, width, height, focal, params, tmp_count):
    ...
    # Convertire tmp_count a int/float prima di passarlo a lsd
    tmp_count_num = int(tmp_count) if isinstance(tmp_count, str) else tmp_count
    lines = lsd(gray, tmp_count_num)
    lsd_output = lines[:, :4]
