...
def save_visualization_image(image, filepath):
    """Save visualization image using matplotlib to ensure proper handling of line drawings."""
    try:
        print(f"Saving visualization image to {filepath}")
        
        # Use matplotlib's savefig for visualization images
        ...
        if os.path.exists(filepath):
            print(f"Visualization file saved successfully: {os.path.getsize(filepath)} bytes")
            return True
        else:
            print("Visualization file not saved!")
            return False
    except Exception as e:
...
for i, row in list(gdf_combined.iterrows())[:5]:  
    ...
    # Save the images using our specialized functions
    print(f"Saving images for Building {building_id}")
    perspective_saved = save_perspective_image(result['image'], output_path)
    vis_saved = save_visualization_image(result['visualization'], vis_path)
    
    if perspective_saved and vis_saved:
        print(f"Successfully saved all images for Building {building_id}")
    else:
        print(f"Warning: Issues saving images for Building {building_id}")
        continue
    ...
    # Create a text file with the scores
    with open(os.path.join(visualization_folder, f"Building_{building_id}{suffix}_scores.txt"), 'w') as f:
        f.write(f"Building {building_id} - Best point: {result['point_name']}\n")
        f.write(f"FOV: {result['fov']:.1f}°\n")
        f.write(f"Combined score: {result['scores']['combined_score']:.3f}\n")
        f.write(f"Parallelism score: {result['scores']['parallelism_score']:.3f}\n")
        f.write(f"Alignment score: {result['scores']['alignment_score']:.3f}\n")
        f.write(f"Horizontal edgelets: {result['scores']['horizontal_edgelets_count']}\n")
        f.write(f"Strong edgelets count: {result['scores']['strong_edgelets_count']}\n")
        f.write(f"Horizontal line density: {result['scores']['horizontal_line_density']:.3f}\n")
