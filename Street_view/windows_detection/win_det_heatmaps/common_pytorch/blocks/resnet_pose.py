...
def init_pose_net(pose_net, cfg):
    if cfg.from_model_zoo:
        _, _, _, name, resnet_model = resnet_spec[cfg.num_layers]
        weights = weights_map[name]
        org_resnet = resnet_model(weights=weights).state_dict()
...
def get_pose_net(network_cfg, ae_feat_dim, num_point_types):
    block_type, layers, channels, name, resnet_model = resnet_spec[network_cfg.num_layers]
