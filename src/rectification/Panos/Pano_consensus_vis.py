...
import os
from .Pano_utils import line_intersection_homo
from numpy import linalg
import matplotlib.pyplot as plt

# qutip is only needed for optional debug visualization
try:
...
from .Pano_visualization import calculate_zenith_point_coordinate, calculate_horizon_coordinates, calculate_hvps, R_roll, R_pitch
...
def draw_consensus_rectified_sphere(hv_points, root):
    """Draw consensus rectified sphere (requires qutip library)."""
    if not QUTIP_AVAILABLE:
        import warnings
        warnings.warn("qutip not installed. Skipping draw_consensus_rectified_sphere.", UserWarning)
        return
    
    b = Bloch()
    # Rimuoviamo la specifica esplicita dei colori, lasciando che la libreria gestisca i colori
...
def draw_center_hvps_rectified_sphere(hv_points, root):
    """Draw center horizontal vanishing points on rectified sphere (requires qutip library)."""
    if not QUTIP_AVAILABLE:
        import warnings
        warnings.warn("qutip not installed. Skipping draw_center_hvps_rectified_sphere.", UserWarning)
        return
