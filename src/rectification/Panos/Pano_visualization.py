...
# qutip is only needed for optional debug visualization (draw_sphere_zenith)
# Import it only if available, otherwise provide a fallback
try:
    from qutip import Bloch, basis
    QUTIP_AVAILABLE = True
except ImportError:
    QUTIP_AVAILABLE = False
    # Provide dummy values so the module can still be imported
    Bloch = None
    basis = None
...
def draw_sphere_zenith(zenith_points, hv_points, root):
    """
    Draw zenith points on a 3D Bloch sphere (requires qutip library).
    If qutip is not installed, this function will skip visualization and log a warning.
    """
    if not QUTIP_AVAILABLE:
        import warnings
        warnings.warn(
            "qutip library not installed. Skipping draw_sphere_zenith visualization. "
