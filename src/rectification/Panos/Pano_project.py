...
import glob
from .Pano_visualization import R_roll, R_pitch, R_heading
from .Pano_new_pano import calculate_new_pano, save_heading_pitch_json, save_heading_only
from .Pano_refine_project import simon_refine
import json
...
def project_facade_for_refine(final_hvps_rectified, im, pitch, roll, im_path, root, tmp_folder, rendering_img_base, tmp_count, save_heading_map='bottom_row_only'):
    """
    ...
    Args:
        ...
        save_heading_map: Mode for saving heading map ('full', 'bottom_row_only', or 'none')
    """
        ...
        for j in range(2):
            ...
            if no_adaptive:
                ...
                if refine_radians != None:
                    ...
                    # ⭐ OTTIMIZZAZIONE HEADING MAP ⭐
                    # Salva solo la bottom row per risparmiare storage (9 KB vs 96 MB)
                    # Il cropping usa solo la bottom row, quindi non c'è perdita di funzionalità
                    if save_heading_map == 'bottom_row_only':
                        # Salva solo l'ultima riga (ground level)
                        ttttt_heading_to_save = ttttt_heading[-1, :]
                        # Salva anche JSON per compatibilità (può essere rimosso in produzione)
                        with open(ttttt_heading_json_path.replace('.npy', '.json'), 'w') as f:
                            json.dump(ttttt_heading_to_save.tolist(), f)
                        np.save(ttttt_heading_json_path, ttttt_heading_to_save)
                    elif save_heading_map == 'full':
                        # Salva l'array 2D completo (legacy mode)
                        with open(ttttt_heading_json_path.replace('.npy', '.json'), 'w') as f:
                            json.dump(ttttt_heading.tolist(), f)
                        np.save(ttttt_heading_json_path, ttttt_heading)
                    elif save_heading_map == 'none':
                        # Non salvare heading map (solo per debug)
                        pass
                    else:
                        # Default: bottom_row_only
                        ttttt_heading_to_save = ttttt_heading[-1, :]
                        np.save(ttttt_heading_json_path, ttttt_heading_to_save)
