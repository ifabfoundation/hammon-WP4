...
GeoDataFrame processor for building geometry operations
"""
...
class GeoDataFrameProcessor:
    ...
    def load_csv(path: Path) -> pd.DataFrame:
        """Read a CSV file with building data."""
        try:
            return pd.read_csv(path)
        except Exception as exc:
            raise SystemExit(f"Unable to read CSV → {exc}") from exc
    ...
    @staticmethod
    def first_two_vertices(edge: LineString) -> Tuple[Point, Optional[Point]]:
    ...
    @staticmethod
    def calculate_cardinal_direction(source: str, target: str) -> str:
