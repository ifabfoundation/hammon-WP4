...
Sky detection and removal from building facade images
"""
...
class SkyCropper:
    ...
    def __init__(self, sky_offset: int = 20):
        ...
        Args:
            sky_offset: Additional offset above the detected roof line
        """
    ...
    def compute_horizontal_gradient_sum(self, image: np.ndarray) -> np.ndarray:
        ...
        Returns:
            Array of gradient sums for each row
        """
    ...
    def find_roof_line(self, gradient_sum: np.ndarray, window_size: int = 11, 
        ...
        Args:
            gradient_sum: Array of gradient sums for each row
            window_size: Size of window for local maxima detection
            threshold_factor: Factor to determine significant gradient changes
        
        Returns:
            Row index representing the roof line
        """
    ...
    def crop_sky_from_image(self, image: np.ndarray, offset: Optional[int] = None) -> Tuple[np.ndarray, int]:
        ...
        Args:
            ...
            offset: Additional offset above the detected roof line
        
        Returns:
            Tuple of (cropped image, crop line position)
        """
    ...
    def visualize_gradient_and_crop(self, image: np.ndarray, crop_line: int) -> np.ndarray:
        ...
        Args:
            ...
            crop_line: The detected crop line
    ...
    def process_and_display_image(self, image_path: str, offset: Optional[int] = None, 
        ...
        Returns:
            Tuple of (original image, cropped image)
        """
        # Load image
    ...
    def process_building_image(self, image: np.ndarray, output_dir: str, filename: str, 
                               save_image: bool = True, image_quality: int = 95) -> np.ndarray:
        """
        ...
        Args:
            image: Input image from BuildingProcessor
            output_dir: Directory to save the final image
            filename: Filename for the output image
            save_image: Whether to save the processed image
            image_quality: JPEG quality (1-100)
            
        Returns:
            Processed image without sky
        """
        try:
            ...
            if save_image:
                ...
                cropped_pil.save(output_path, format='JPEG', quality=image_quality)
